...
Admin and Staff edit planned departure passengers, both drivers, and event vehicles for every event. One Add dialog groups active Students, Staff, and Vehicles and submits up to 20 combined selections atomically. Both roles may directly edit effective return passengers and drivers after Depart and through `return_started`; departure facts remain snapshot-based and read-only. The default destination inside Admin Configuration > Vehicles remains Admin-only.
