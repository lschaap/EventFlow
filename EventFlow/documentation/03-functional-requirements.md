...
- REQ-132: After Depart and before Start Return, active approved Admin and Staff may individually or atomically bulk-move up to 100 return passengers only among active `departed`/`arrived_at_event` trips and Return Unassigned; departure assignments remain immutable.
- REQ-133: Start Return is a per-vehicle, reviewed `arrived_at_event -> return_started` transaction with request-time timestamp/user audit and an immutable original return snapshot using the configured destination. Unassigned and overcapacity are warning-only and require explicit confirmation.
- REQ-134: Active approved Admin and Staff may directly update effective return passengers and drivers after Depart and through `return_started`. These bounded writes preserve all departure facts and original snapshots, do not create correction-history records, and are unavailable after Returned.
- REQ-136: Draft and confirmed events expose one Add dialog for selecting active students, staff, and vehicles. One atomic Add operation accepts at most 20 total selections, rejects duplicates/conflicts, updates participant counts and dietary state, and either commits every selected item or writes nothing.
- REQ-135: Returned and automatic event completion remain unimplemented. WhatsApp and generalized vehicle movements remain post-MVP.
