...
try {
  ...
  const { buildOriginalReturnSnapshot, effectiveRosterDiffers, returnMoveRequiresCorrection, returnTargetIsEligible, startReturnBlockingError, startReturnReviewToken, startReturnWarnings } = await import('../.transportation-test/services/returnPlanning.js')
  ...
  assert.equal(returnTargetIsEligible('departed', false), true, 'ordinary return editing accepts departed targets')
  assert.equal(returnTargetIsEligible('arrived_at_event', false), true, 'ordinary return editing accepts arrived targets')
  ...
  assert.match(startReturnBlockingError('in_progress', { ...returnTrip, returnDriverStaffId: null }), /driver/, 'missing return driver blocks Start Return')
  assert.match(startReturnBlockingError('completed', returnTrip), /in progress/, 'completed event cannot Start Return')
  const returnOccupants = [{ kind: 'staff', personId: 'driver', relationshipId: 'event__driver', displayName: 'Driver', departureVehicleId: 'van-a', returnVehicleId: 'van-a' }, { kind: 'student', personId: 'student', relationshipId: 'event__student', displayName: 'Student', departureVehicleId: 'van-a', returnVehicleId: 'van-a' }]
  const returnBase = { eventId: 'event', eventName: 'Museum', eventStatus: 'in_progress', eventUpdatedAtMillis: 5, tripId: 'event__van-a', tripUpdatedAtMillis: 20, vehicleId: 'van-a', vehicleName: 'Van A', returnDriverStaffId: 'driver', returnDriverName: 'Driver', destination: 'Mill Village', occupants: returnOccupants, studentCount: 1, staffCount: 1, totalOccupants: 2, capacity: 2, overCapacityBy: 0, unassignedReturnCount: 0, arrivedAtEventMillis: 20 }
  const returnToken = startReturnReviewToken(returnBase)
  assert.notEqual(returnToken, startReturnReviewToken({ ...returnBase, occupants: returnOccupants.slice(0, 1), totalOccupants: 1 }), 'roster changes invalidate Start Return review')
  const originalReturn = buildOriginalReturnSnapshot({ ...returnBase, reviewToken: returnToken }, 'staff', 'SERVER_TIME')
  assert.equal(originalReturn.destination, 'Mill Village', 'original snapshot records configured return destination')
  assert.equal(originalReturn.startedByUserId, 'staff', 'original snapshot records Start Return user')
  assert.equal(effectiveRosterDiffers(originalReturn, returnOccupants), false, 'unchanged effective roster matches original snapshot')
  assert.equal(effectiveRosterDiffers(originalReturn, returnOccupants.slice(0, 1)), true, 'corrected effective roster differs without mutating original snapshot')
