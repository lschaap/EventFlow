...
import { returnTargetIsEligible, snapshotOccupantKeys } from '../services/returnPlanning'
import { correctReturnAssignments, listReturnCorrections } from '../services/returnCorrections'
import type { EventParticipantRecord, EventRecord, EventStaffParticipantRecord, EventVehicleTripRecord, ReturnRosterCorrectionRecord, StaffRecord, StudentRecord, VehicleRecord } from '../types/models'
...
export default function VehicleTripPlanning({ event, vehicles, staff, onParticipantsChanged }: { event: EventRecord; vehicles: VehicleRecord[]; staff: StaffRecord[]; onParticipantsChanged?: () => Promise<void> }) {
  ...
  const load = async () => {
    const [nextTrips, people, studentParts, nextStaffParts, nextCorrections] = await Promise.all([listActiveEventVehicleTrips(eventId), listStudents(), listParticipantsForEvent(eventId), listStaffParticipantsForEvent(eventId), listReturnCorrections(eventId)])
    setTrips(nextTrips)
  ...
  const returnSelectionNeedsCorrection = returnSelectedPeople.some((person) => {
    const source = person.returnVehicleId ? trips.find((trip) => trip.vehicleId === person.returnVehicleId) : null
    return event.status === 'completed' || source?.stage === 'return_started' || source?.stage === 'returned'
  })
  ...
  return <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
    ...
    {trips.some((trip) => trip.stage !== 'planned') ? <section className="mt-6 rounded-2xl border border-indigo-200 bg-indigo-50/40 p-4">
      ...
      <div className="mt-4 space-y-4">{returnGroups.filter((group) => group.vehicleId == null || trips.find((trip) => trip.vehicleId === group.vehicleId)?.stage !== 'planned').map((group) => {
        ...
        const correctionMode = trip?.stage === 'return_started' || trip?.stage === 'returned'
        ...
        return <article key={`return:${group.vehicleId ?? 'unassigned'}`} className="rounded-2xl border bg-white p-4">
          ...
          <div className="mt-3 space-y-2">{group.occupants.length ? group.occupants.map((person) => {
            const sourceTrip = person.returnVehicleId ? trips.find((candidate) => candidate.vehicleId === person.returnVehicleId) : null
            const personCorrection = event.status === 'completed' || sourceTrip?.stage === 'return_started' || sourceTrip?.stage === 'returned'
